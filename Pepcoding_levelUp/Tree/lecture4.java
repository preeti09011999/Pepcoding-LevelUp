...
public class lecture4{
    ...
    public static void main(String args[]){
        ...
        construct_binary(preorder,0,n-1,inorder, 0, n-1);
    }
...
// when node.right is null then we will check for the first parent of the node whose data is greater than node.data
    ...
    public static Node construct_binary(int[] preorder, int psp, int pep,int[] inorder, int isp, int iep) {
        if(psp > pep) return null;
        int idx = isp;
        while(inorder[idx] != preorder[psp]) idx++;
        int len = idx - isp;
        Node node = new Node(preorder[psp]);
        node.left = construct_binary(preorder, psp + 1, psp + len, inorder, isp, idx - 1);
        node.right = construct_binary(preorder, preorder, psp + len + 1, pep, inorder, idx + 1, iep);
        return node;
    }

    // construct binary tree using inorder and postorder
    public static Node construct_binary2(int[] postorder, int psp, int pep,int[] inorder, int isp, int iep) {
        if(psp > pep) return null;
        int idx = isp;
        while(inorder[idx] != postorder[pep]) idx++;
        int len = idx - isp;
        Node node = new Node(preorder[psp]);
        node.left = construct_binary(preorder, psp, psp + len - 1, inorder, isp, idx - 1);
        node.right = construct_binary(preorder, preorder, psp + len , pep - 1, inorder, idx + 1, iep);
        return node;
    }
}